...
class DataRecorder(Node):
    ...
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.get_logger().info(
            "DataRecorder 节点启动 ✅  save_root=%s  task=%s",
            args.save_root, args.task,
        )
    ...
    def _connect_cameras(self) -> None:
        ...
        try:
            ...
            self.get_logger().warning("鱼眼相机异常: %s", exc)
        ...
        try:
            ...
            self.get_logger().warning("RealSense 相机异常: %s", exc)
    ...
    def _stop_recording(self) -> None:
        ...
        if ep:
            # 在后台线程完成文件写入，不阻塞 ROS 回调
            threading.Thread(target=ep.close, daemon=True).start()
